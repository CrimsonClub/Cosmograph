...
public final class ItemPlaceholder {
     ...
     */
    public static Component apply(Cosmograph plugin, Player player, Component rendered, boolean withHover) {
        if (!plugin.getConfig().getBoolean("use-item-placeholder", false)
                || !player.hasPermission("cosmograph.itemplaceholder")) {
            return rendered;
        }
    ...
    private static Component displayName(Cosmograph plugin, ItemStack item) {
        ...
        if (meta != null && meta.hasDisplayName()) {
            if (plugin.isPaper()) {
                ...
                return Cosmograph.getLegacySerializer().deserialize(meta.getDisplayName());
            }
        }

...
public final class ChatFormatService {
    ...
    private static final String MESSAGE_TAG_PREFIX = "cosmographmsg";
    private static final String STYLE_INNER_TAG = "cosmographstyled";
    ...
    private volatile String whisperFormat;

    public ChatFormatService(Cosmograph plugin) {
        this.plugin = plugin;
    ...
    public void reload() {
        ...
        this.whisperFormat = config.getString("whisper-format",
                "<dark_gray>[<light_purple>MSG</light_purple>] <sender> <gray>-></gray> <recipient><dark_gray>:<reset> {message}");
    }
    ...
    /** Renders a private message using the configured trusted whisper template. */
    public Component renderWhisper(Player sender, Player recipient, Component message) {
        String tag = MESSAGE_TAG_PREFIX + UUID.randomUUID().toString().replace("-", "");
        String format = whisperFormat.replace(MESSAGE_TOKEN, "<" + tag + ">")
                .replace("{sender}", sender.getName())
                .replace("{recipient}", recipient.getName());
        return trustedMiniMessage.deserialize(format, Placeholder.component(tag, message));
    }

    /**
     * Renders an operator-authored template (e.g. join/quit message) for a player. No player
     * chat text is involved; {@code extra} resolvers can inject pre-built components.
     */
